...
_Amended 2026-08-03 (Michael's UX feedback): two-pane desktop layout, uncropped photos, arrow navigation, background scroll lock. This section is the current binding markup; the original single-column spec is history._
- `w-full max-w-4xl lg:max-w-6xl p-0 border border-hairline bg-white text-ink max-h-[calc(100dvh-2rem)] overflow-y-auto lg:overflow-hidden overscroll-contain`, `::backdrop rgba(15,20,25,.6)`. Inner wrapper: `relative lg:grid lg:grid-cols-[3fr_2fr] lg:h-[min(46rem,calc(100dvh-2rem))]` — gallery left, info right on desktop so the info is visible on open; stacked on mobile with the main image capped (`max-h-[45dvh]`) so title/price peek above the fold. `document.body.style.overflow` is set to `hidden` on open and cleared on close (background never scrolls); both scroll containers carry `overscroll-contain`.
- Gallery pane (`bg-surface lg:flex lg:flex-col lg:min-h-0`): main img `w-full aspect-[4/3] max-h-[45dvh] object-contain lg:absolute lg:inset-0 lg:h-full lg:max-h-none lg:aspect-auto` (uncropped, letterboxed on `bg-surface`) inside a `relative lg:flex-1 lg:min-h-0` box that also holds prev/next arrow buttons (`#property-modal-prev`/`-next`, `absolute` mid-left/right, `bg-white/90`, chevron svgs, `hidden` when <2 photos, wrap-around; ArrowLeft/ArrowRight keys work too) and a photo counter `#property-modal-photo-count` (`absolute bottom-3 right-3 bg-ink/70 text-white text-xs`, "N / M", hidden when <2 photos). Thumbnail strip below: `flex gap-2 p-2 overflow-x-auto bg-white`, thumbs are `<button type="button" aria-label="Photo N">` wrapping `w-20 h-14 object-cover` imgs, `opacity-60`, active `opacity-100`, `focus-visible:ring-2 ring-accent`.
- Info pane `p-8 sm:p-10 lg:min-h-0 lg:overflow-y-auto overscroll-contain`:
